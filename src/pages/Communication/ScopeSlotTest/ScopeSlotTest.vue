<template>
  <div>
    ...
    <!-- 决定父组件传递怎样标签结构的数据在子组件中 -->
    <List :data="todos">
      <!-- 
        slot-scope属性：用来指定接收子组件中的<slot>的</slot>所有的标签属性的对象
        scope对象的结构：{row:todo对象, $index:下标值}
      -->
      <template slot-scope="scope">
        <span v-if="scope.row.isComplete" style="color:gold">{{scope.row.text}}</span>
    ...
    <List :data="todos">
      <template slot-scope="{row, $index}">
        <span :style="{color:$index%2==1?'gold':'thistle'}">{{$index+1}}{{row.text}}</span>
      </template>
    </List>
  </div>
</template>
