...
export default {
  ...
  methods: {
    pay() {
      QRCode.toDataURL(this.payInfo.codeUrl) // 二维码地址数据
        .then(url => {
          ...
          this.intervalId = setInterval(() => {
            this.$API.reqPayStatus(this.$route.query.orderId)
              .then(result => {
                // 请求成功
                // 支付成功（没有主动点击"我已支付成功"）
                // 关闭对话框 提示成功信息 自动跳转到支付成功界面  删除购物车中已勾选的商品项 清除定时器
                if (result.code === 200) {
                  this.$msgbox.close();// 关闭对话框
                  this.$message({
                    type: "success",
                    message: "支付成功!"
                  });
                  this.$router.replace("paysuccess");
                  this.$store.dispatch("deleteChecked");
                  clearInterval(this.intervalId);
                } else {
                  // 未支付
                }
