  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div
        ...
        v-for="userAddressInfo in tradeInfo.userAddressList"
        :key="userAddressInfo.id"
      >
        <span
          class="username"
          :class="{selected: userAddressInfo===selectedAddress}"
          @click="selectedAddress=userAddressInfo"
        >{{userAddressInfo.consignee}}</span>
        <p @click="selectedAddress=userAddressInfo">
          <span class="s1">{{userAddressInfo.userAddress}}</span>
          <span class="s2">{{userAddressInfo.phoneNum}}</span>
          <span class="s3" v-if="userAddressInfo.isDefault === '1'">默认地址</span>
        </p>
      </div>
...
export default {
  ...
  watch: {
    "tradeInfo.userAddressList"(value) {
      ...
      // 指定为选中地址
      this.selectedAddress = defaultAddress || {}; // 如果没有默认地址，就是{}
    }
  },
  mounted() {
  ...
  methods: {
    async submitOrder() {
      ...
      const orderInfo = {
        ...
        paymentWay: "ONLINE",
        orderComment, // 用户留言
        orderDetailList: detailArrayList // 订单列表
      };
      // 发请求
      try {
